import {Button, Card, CardBody, CardHeader, Col, Collapse, FormCheck, ListGroup, ListGroupItem} from "react-bootstrap";
            ...
            <Col md={3} className="position-sticky">
                <ListGroup>
                    <ListGroupItem className="d-flex justify-content-between align-items-center">
                        <span>Username:</span>
                        <input type="text" name="usernamePrompt" value={filters.usernamePrompt} className="form-control" placeholder="Username"
                               onChange={(e) => updateFilterUsername(e.target.value)}/>
                        <FontAwesomeIcon icon={faQuestion} style={{color: 'orange', cursor: 'pointer'}} onClick={() => notifySuccess('Your prompt is used as prefix to search usernames')} />
                    </ListGroupItem>
                    <ListGroupItem className="d-flex justify-content-between align-items-center">
                        <span>Email:</span>
                        <input type="text" name="emailPrompt" value={filters.emailPrompt} className="form-control" placeholder="Email"
                               onChange={e => updateFilterEmail(e.target.value)} />
                        <FontAwesomeIcon icon={faQuestion} style={{color: 'orange', cursor: 'pointer'}} onClick={() => notifySuccess('Your prompt is used as prefix to search email')} />
                    </ListGroupItem>
                    <ListGroupItem className="d-flex justify-content-between align-items-center">
                        <span>Telegram:</span>
                        <input type="text" name="telegramPrompt" value={filters.telegramPrompt} className="form-control" placeholder="Telegram"
                               onChange={e => updateFilterTelegram(e.target.value)} />
                        <FontAwesomeIcon icon={faQuestion} style={{color: 'orange', cursor: 'pointer'}} onClick={() => notifySuccess('Your prompt is used as prefix to search telegram')} />
                    </ListGroupItem>
                    ...
                    <ListGroupItem className="d-flex justify-content-between">
                        <Button color="primary" onClick={getUsers}>Filter</Button>
                        <Button color="primary" onClick={dropFilters}>Drop Filters</Button>
                    </ListGroupItem>
                </ListGroup>
            </Col>
            <Col md={6}>
                            ...
                            <Card className="mb-3 w-100"  key={crypto.randomUUID()}>
                                <CardHeader onClick={() => toggleUser(user.id)} style={{ cursor: 'pointer' }}>
                                    <div className="d-flex justify-content-between align-items-center">
                                        <span>Username: {user.username} | Status: {user.status} | Online: {String(user.loginTime > user.logoutTime).toUpperCase()}</span>
                                ...
                                <Collapse in={selectedUserId === user.id}>
                                    <CardBody>
                                        ...
                                        <Button onClick={() => updateUser({status: user.status === 'ENABLED' ? 'DISABLED' : 'ENABLED', id: user.id})} className="my-1">
                                            {
                                                user.status === 'ENABLED' ?
                                                    'Disable User'
                                                    :
