    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (categoryId) {
                    [categoriesData, productsData] = await Promise.all([getCategories(CategoryFilter.build({page: categoriesPage, parentCategoryId: categoryId.toString()})),
                        getProducts(ProductFilter.build({page: productPage, categoryId: categoryId.toString()}))]);
                } else {
                    [categoriesData] = await Promise.all([getCategories(CategoryFilter.build({page: categoriesPage, parentCategoryId: null}))]);
                }
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                let productsData;
                if (categoryId) {
                    [productsData] = await Promise.all([getProducts(ProductFilter.build({page: productPage, categoryId: categoryId.toString()}))]);
                }
                if (productsData) {
                    setProducts(productsData);
                }
                    ...
                    <Row style={{height: '100%', overflow: 'scroll', width: '100vw'}}>
                        ...
                        <Col style={{
                            ...
                            <Container fluid style={{width: '90%'}}>
                                ...
                                <Categories categories={categories} isSubCategoryOpened={isSubCategoryOpened} categoriesPage={categoriesPage} setCategoriesPage={setCategoriesPage} productPresent={products.length > 0}/>
                                {
                                    products.length > 0 &&
                                ...
                                }
                                {
                                    ...
                                    <Products products={products} productPage={productPage} setProductPage={setProductPage} categoriesPresent={categories.length > 0}/>
                                }
                            </Container>
                        </Col>
